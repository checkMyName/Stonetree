...
const sectionAnimationSelector = '.section-animation';
...
export function sectionAnimation() {
  ...
  const getSectionsOffsetTop = () => {
    getAll(`${pageWrapper} ${sectionAnimationSelector}`).forEach((element, i) => {
  ...
  const sectionsAnimations = () => {
    ...
    let panels = gsap.utils.toArray(sectionAnimationSelector);
